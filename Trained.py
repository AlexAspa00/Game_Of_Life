...
class Food:
    def __init__(self, rad):
        ...
        self.pos = [int(random.randint((screen_width - rad) - 300, (screen_width - rad))), int(random.randint(rad, screen_height - rad))]
...
class Criatura1:
    ...
    def check_collision(self, cars, screen):
        ...
        if (self.pos[0] + self.radius >= 1200) or (self.pos[0] < self.radius) or (self.pos[1] + self.radius >= screen_height) or (self.pos[1] < self.radius):
            self.is_alive = False
        else:
...
class Criatura2:
    ...
    def get_reward(self):
        ...
        return (((time.time() - self.time_after_eating) * (aux)) / 10 + (time.time() - self.time) * 0.1)
...
def run_sp1(genomes):
    ...
    while True:
        ...
        for index, car in enumerate(total_criatures[0]):
        ...
        i = 0
        for i, car in enumerate(total_criatures[0]):
            if car.get_alive():
                remain_cars += 1
...
def run_sp2(genomes):
    ...
    while True:
        ...
        # Input my data and get result from network
        for index, car in enumerate(total_criatures[1]):
            output = nets[index].activate(car.get_data())
        ...
        i = 0
        for i, car in enumerate(total_criatures[1]):
            if car.get_alive():
                remain_cars += 1
                car.update(cars_aux, screen)
                genomes[i][1].fitness += car.get_reward()
...
def run_neat(config,id_specie):
    ...
    if (id_specie == 0):
        ...
        with open("Results_Sp1/" + test + "/winner.pkl", "rb") as f:
            genome = pickle.load(f)
            # Convert loaded genome into required data structure
            genomes = [(1, genome), (2,genome), (3, genome), (4,genome), (5, genome), (6,genome), (7, genome), (8,genome), (9, genome), (10,genome)]
            while (1):
                run_sp1(genomes)
        ...
        test = "20"
        # Unpickle saved winner
        with open("Results_Sp2/" + test + "/winner.pkl", "rb") as f:
            genome = pickle.load(f)
            # Convert loaded genome into required data structure
            genomes = [(1, genome), (2,genome), (3, genome), (4,genome), (5, genome), (6,genome), (7, genome), (8,genome), (9, genome), (10,genome)]
            while (1):
