'''
This file is used to train the blue creature in a completely static scenario.
'''
import pickle
...
def run_sp2(genomes, config):
    ...
    for id, g in genomes:
        ...
        # Init my creature
        total_criatures[1].append(Criatura2(screen, time.time(), u + 100, 25, 0, 0, 255, food_aux, u, g))
        u = u + 1
    ...
    while True:
        ...
        for index, criatura in enumerate(total_criatures[1]):
            output = nets[criatura.idgen].activate(criatura.get_data())

            criatura.angle += round(output[0])
        ...
        for i, criatura in enumerate(total_criatures[1]):
            if criatura.get_alive():
                ...
                criatura.gen.fitness = criatura.get_reward()
...
def run_neat(config):
    ...
    node_names = {-1: 'Radar1', -2: 'Radar2', -3: 'Radar3', -4: 'Radar4', 0: 'Angle'}
